...
```text
//10Base以太网最多五个网段，4个中继器，3台主机
//总的争用期是不变的，有时需要扣除中继器的时延，会导致链路距离变短
//中继器两端数据传输速率必须相同
//BGP报文：OPEN，KEEPALIVE，UPDATE，NOTIFICATION
//DHCP在分配IP地址时，会告知默认网关的IP地址，默认DNS的IP地址，但不会提供默认网关的MAC地址
//每换一段链路之后的源MAC和目的MAC都会改变，IP和端口号在遇到NAT的时候可能会改变
```

<span style="color:#258a4b;font-weight:700">补充说明</span>

- 经典以太网的 5-4-3 规则适用于共享式半双工以太网的特定拓扑：最多 5 个网段、4 个中继器，且最多 3 个可连接站点的混合网段；不应直接套用于现代交换式以太网。
- 中继器工作在物理层，通常要求连接两端采用兼容的数据传输速率与信号规范。中继器时延会占用争用期预算，从而限制最大网络直径。
- DHCP 可下发默认网关和 DNS 服务器的 IP 地址。主机需要发送数据到网关时，仍需在本地链路上通过 ARP（IPv4）解析网关接口的 MAC 地址。
- 跨越路由器时，链路层帧会被重新封装，因此源 MAC 和目的 MAC 会按下一跳改变；通常端到端 IP 地址和传输层端口保持不变，NAT 场景则可能改写它们。
