...
```text
//文件名在 open 时已经解析过、转成 fd 了，read 只用 fd
//系统调用命令本身应该在用户态就能执行
//建立进程：OS启动，用户请求，进程中出现了系统调用
//阻塞是进程自身的一种主动行为，调用阻塞原语。
//操作系统引导，开机时，PC强制初始化为BIOS的入口地址（在ROM中），BIOS有一条无条件转移指令转移到BIOS的主体部分
//BIOS也会初始化创建中断向量表
//BIOS去找启动盘的主引导记录MBR，MBR会把磁盘的分区引导记录PBR加载到内存，执行PBR
//内核态返回用户态通常通过中断、异常或系统调用的返回指令恢复现场和用户态执行环境完成的
//静态重定位，用户程序装入内存时，直接一次性到位，此时程序不能再移动了
//动态重定位，当需要访问内存单元时，再进行内存转换，需要基地址寄存器
```

<span style="color:#258a4b;font-weight:700">补充说明</span>

- `open` 按路径查找文件并在内核中建立打开文件相关表项，向进程返回文件描述符；之后 `read`、`write` 等操作主要通过该文件描述符定位已打开文件。
- 应用程序在用户态执行系统调用的入口指令；执行系统调用服务例程时，CPU 会陷入内核态，完成后再返回用户态。
- 进程创建可由系统初始化、用户请求或现有进程执行创建类系统调用等事件触发。
- 静态重定位在装入时完成地址调整；动态重定位在运行时进行，便于程序在内存中移动。
