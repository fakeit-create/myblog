(() => {
  ...
  function prettyInternalUrl(value) {
    const valueText = String(value || '').trim();
    if (!valueText) return '';

    // Repair both newly generated links and stale knowledge-data.json entries,
    // e.g. /wiki/docs/questions-2016.html -> /wiki/docs/questions-2016/.
    // Absolute links are rewritten only when they point to the current site.
    let parsed;
    try {
      parsed = new URL(valueText, location.href);
    } catch (_) {
      return valueText;
    }
    if (parsed.origin !== location.origin) return valueText;
  ...
  function normalise(raw) {
    const documents = Array.isArray(raw?.documents)
      ? raw.documents.filter(Boolean).map(document => ({
        ...document,
        url: prettyInternalUrl(document.url)
      }))
      : [];
